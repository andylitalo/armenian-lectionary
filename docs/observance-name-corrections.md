...
**Pentecost+7.** English's own raw text is byte-identical to Pentecost+14's — both read
`Second Sunday after Pentecost` — on the theory that Pentecost itself is implicitly the
first Sunday, so the count floors at 2 and never states a "First Sunday after Pentecost"
at all. The Armenian disagrees with that theory on its own terms: it numbers these Sundays
`Ա/Բ/Գ...` (First/Second/Third...) straight through, never repeating a number —
`Ա կիւրակէ զկնի Հոգեգալստեան` on Pentecost+7, `Բ...` on Pentecost+14, confirmed against the
source's own Armenian cache and stable across every English-cached year (2001–2026, no
exceptions). The more specific statement stands: Pentecost+7 is now `First Sunday after
Pentecost`, with its own id (`first_sunday_after_pentecost`) rather than sharing
`second_sunday_after_pentecost` with Pentecost+14.

This one had previously been resolved the other way — the observance catalog forced
*both* weeks to the single canonical `Բ` (Second), treating the English floor-at-2 reading
as authoritative and the Armenian's distinct `Ա` as a spelling variant to fold away (see
`tests/test_language.py`'s now-renamed `test_first_and_second_sunday_after_pentecost_
ordinals`, and `dev/observance_ids._RECURRING_OBSERVANCES`'s now-removed
`second_sunday_after_pentecost` entry, which had declared the twice-a-year recurrence "by
design"). Registered as
`source_corrections.first_sunday_after_pentecost_label`/`_label_hy` and regenerated by a
matching `engine._POSITION_FAMILIES` entry (the Pentecost+7 tuple floors at 1, not 2), so
the stored and overlaid labels agree, the same shape as the Illuminator/Nisibis fasts in
§5 — except here the source's OWN English is what was one-string-for-two-observances,
not just a bare generic marker.
...
### The Transfiguration family's definite article

`Eve of the Fast of Transfiguration`, `Second`/`Third day of Transfiguration` and the
Second–Seventh Sundays (`Nth Sunday of Transfiguration`) were missing the article every
sibling fast/day/Sunday family already carries: `the Fast of the Holy Cross`, `the Fast of
the Assumption`, the family's own `Nth day of the Fast of the Transfiguration` (the fast
days, already correct), and — after the fix directly below — `Nth Sunday of the Holy
Cross`/`the Assumption`. Same family-consistency reasoning as the Assumption article fix
above, not a self-contradiction in any one row. Two literals lived in `engine.py` rather
than the table (`_EVE_FAMILIES`'s Transfiguration entry and `_POSITION_FAMILIES`'s TR-anchor
day/Sunday templates) and needed an `engine.py` edit alongside the review-row edit, the same
shape the Great Lent Sunday connector fix above needed for its Sixth Sunday.

### Why զկնի can sit over both — `Sunday after Pentecost` vs `Sunday of the Transfiguration`

Four families share one construction, `{ord} Sunday <preposition> <Anchor>`, generated by
the same `_POSITION_FAMILIES` machinery on four different movable anchors (Pentecost, the
Holy Cross, the Assumption, the Transfiguration). All four render their Armenian the same
way — `<ordinal> կիւրակէ զկնի <Anchor>` — but the source's own **English** preposition and
the family's own **ordinal floor** disagree with each other family-by-family, and the
served English used to paper over that by picking one preposition (`of`) for all four
regardless. That was wrong for Pentecost and right, by accident, for the other three.

**The evidence is the floor, not the word.** `_POSITION_FAMILIES`'s ordinal-adjustment
column fixes, for each family, which ordinal the first Sunday after the anchor gets:

| Family | First Sunday after the anchor is called | Adjustment |
|---|---|---|
| Pentecost | **First** Sunday after Pentecost (Pentecost+7) | `0` |
| Holy Cross | **Second** Sunday after the Holy Cross (EX+7) | `1` |
| Assumption | **Second** Sunday after the Assumption (AS+7) | `1` |
| Transfiguration | **Second** Sunday after the Transfiguration (TR+7) | `1` |

Only Pentecost counts the way "Nth Sunday **after** X" reads in plain English: the anchor
itself is Sunday zero, and the next Sunday is truly the first one after it. The other three
skip straight to "Second" — the anchor's own Sunday is implicitly Sunday **one** of the
sequence, exactly the way `Second Sunday of Great Lent`/`of Advent`/`of Eastertide` count
(all bare genitives, no զկնի, and all confirmed exact against the source with the same
floor-1-is-the-feast-itself logic). A family that starts counting at "Second" is a family
counting Sundays **of** a season the anchor opens, not Sundays elapsed **after** a fixed
point — the Tōnats'oyts's own Eastertide/Great Lent/Advent families say exactly this with a
bare genitive and no զկնի at all.

So Armenian's `զկնի` genuinely covers both readings, and the surface word alone
does not disambiguate which one a given family means:

- **[Ա կիւրակէ զկնի] Հոգեգալստեան** — զկնի binds the *ordinal*: "the first Sunday after
  Pentecost." Floor 0. English `after` is exact.
- **[Բ կիւրակէ] [զկնի Վարդավառի]** — զկնի names the *season*: "the second Sunday of the
  [period] after the Transfiguration," counted from the feast itself as Sunday one. Floor 1.
  English `of` is exact; `after` would imply a floor the family does not use.

The source relies on the reader already knowing the liturgical laydown to pick the right
parse — genuinely ambiguous on the page, unambiguous once the ordinal floor is known. And by
the Tōnats'oyts's own habit, an **inclusive** count (a season named from its own opening
feast) is elsewhere always a bare genitive, never զկնի — Eastertide, Great Lent and Advent
all are. The three inclusive-count families using զկնի anyway (Holy Cross, Assumption,
Transfiguration) are the loose usage, and it is in the **Armenian**, not in the English this
project serves: rendering all three as `of` is not a translation of `զկնի`, it is a
translation of what the family actually counts.

**Correction applied.** Pentecost's six Sunday rows were served `Nth Sunday **of**
Pentecost` — matching neither the source's own English (`after`, confirmed on every raw
occurrence, 2001–2026) nor the family's own floor (0, i.e. `after` is exactly right).
Corrected to `after`, which now matches the source's raw text component-for-component and
needed no `engine.py` template beyond the literal word swap. Transfiguration's six Sunday
rows were already served `of`, unquestioned since the row was first registered — the same
blanket choice that made Pentecost wrong made Transfiguration right, by accident rather than
by the floor-1 argument above. Per that argument this is the one family-appropriate case
where the served English should NOT be a bare copy of the source's own preposition (`after`),
so these were left at `of` and only gained the missing article (previous section). Holy Cross
(ids
`second_sunday_of_the_holy_cross` … `eleventh_sunday_of_the_holy_cross`) and Assumption
(`second_sunday_of_the_assumption` … `fifth_sunday_of_the_assumption`) already read `of` and
needed no English change — only the Assumption id inconsistency below.

Transfiguration/Holy Cross/Assumption keeping `of` is the "second, weaker warrant" this
document's introduction names — matching how the rest of the corpus already spells the
identical construction, not the source contradicting itself — which is exactly
why it needed writing up rather than shipping as a bare `reviewed correction` boilerplate
note.

### Observance ids renamed pre-release

`second_sunday_after_transfiguration` … `seventh_sunday_after_transfiguration` (6 ids) and
`third_sunday_after_assumption`, `fourth_sunday_after_assumption` (2 ids) were minted before
the floor-1 argument above was noticed, so they say `after` even though their own served
text has always read `of` (Transfiguration: since the row was first registered; Assumption:
since "The Assumption family's definite article" above). Renamed to
`second_sunday_of_the_transfiguration` … `seventh_sunday_of_the_transfiguration` and
`third_sunday_of_the_assumption`, `fourth_sunday_of_the_assumption`, matching the served
text and the sibling Holy Cross family, which was minted correctly the first time
(`second_sunday_of_the_holy_cross`, …). The Assumption family's other two ids
(`second_sunday_of_the_assumption`, `fifth_sunday_of_the_assumption`) were already right —
only the two minted before that family's own article-normalization pass were inconsistent
with their own siblings.

CLAUDE.md's id-immutability rule ("never change an id") does not apply here: these ids had
never shipped in a tagged PyPI release, the exact window `_RETIRED_IDS`'s own comment
carves out ("Retiring was only available because the catalog's keys had never been served as
ids at this point... After that, an id here keeps meaning the same observance forever").
Retired under that carve-out, in `build_observance_catalog._RETIRED_IDS`, with the reason —
the normal declaration path, not a special case.
...
| `Discovery of the Relics of St. Grigoris the Catholicos of Aghvank...` | ended at `...the Seven Herbivorous Hermits` | `..., who reposed at the Monastery of Innaknya` | The source's own Armenian, `...որք յԻննակնեայ վանսն կատարեցան`. Not a self-contradiction landing on this row so much as an outright omission of one: the earlier `approved_en` simply stopped translating partway through the sentence, dropping the closing clause entirely rather than rendering it differently. `Herbivorous Hermits` (below) stands; see the closed question below. Transliteration corrected from an initial `Innaknia` to `Innaknya`, and the trailing period dropped (the corpus does not otherwise punctuate a component as a sentence), on request. |
...
The one Armenian-side fold in this section — every other row above is an English
correction. The source prints `ԾԱՂԿԱԶԱՐԴ` in full capitals but its own English for the same
day, `Palm Sunday`, is plain title case: an asymmetry between the source's two languages for
one component, not a self-contradiction of fact.

Compared against the corpus's only other consistent ALL-CAPS treatment — the eight Great
Feasts of the Lord, each shouted in **both** languages (`ASCENSION OF OUR LORD JESUS
CHRIST`/`ՀԱՄԲԱՐՁՈՒՄՆ…`, `NATIVITY AND THEOPHANY…`/`ՏՕՆ ԾՆՆԴԵԱՆ…`, `PRESENTATION…`/
`ՏԵԱՌՆԸՆԴԱՌԱՋ`, `RESURRECTION…(Easter Sunday)`/`ՅԱՐՈՒԹԻՒՆ…(Զատիկ)`, `PENTECOST (Fiftieth day
of Eastertide)`/`ՀՈԳԵԳԱԼՈՒՍՏ…`, `TRANSFIGURATION OF OUR LORD (Vardavar)`/
`ՊԱՅԾԱՌԱԿԵՐՊՈՒԹԻՒՆ…`, `ASSUMPTION OF THE HOLY MOTHER OF GOD`/`ՎԵՐԱՓՈԽՈՒՄՆ…`, `EXALTATION OF
THE HOLY CROSS`/`ԽԱՉՎԵՐԱՑ`) — Palm Sunday does not belong to that tier: its own English
confirms it, matching the pattern the other eight all follow (all-caps in both languages,
or neither). The Armenian all-caps reads as a source styling slip rather than a second,
undocumented rank marker. Folded to sentence case, `Ծաղկազարդ`, matching how every other
non-Great-Feast Armenian component in the corpus is cased (e.g. `Երկրորդ ծաղկազարդ`, "Second
Palm Sunday", already lowercases the same word mid-string).
