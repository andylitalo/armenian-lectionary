...
all — a placeholder leak `tests/test_observance_contract.py` forbids. Giving both fasts a real
...
`tests/test_observance.py::TestDecemberNinthFastMarker`,
`tests/test_observance_name_raw.py` (English, ratchets unmoved — see
`dev.source_corrections.expected_fast_marker_components`, which every English-side dev
tool reconciles through), and `tests/test_observance_name_hy_raw.py` (Armenian; its
