...
| [`feast-name-corrections.md`](feast-name-corrections.md) | sacredtradition.am (English vs. its own Armenian) | Every place the engine deliberately departs from the source's feast text, with the evidence for each — plus the questions still open. Companion to [`dev/feast_name_review.tsv`](../dev/feast_name_review.tsv), the approved-name ground truth. |
