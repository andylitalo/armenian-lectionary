...
# them unconditionally instead. See docs/observance-name-corrections.md.
...
_POSITION_FAMILIES = (
    ...
    # the source's own "day of Pentecost" wording; see docs/observance-name-corrections.md.
    ...
    # docs/observance-name-corrections.md section 1); either way, no marker is now served.
    ...
    # wins). See docs/observance-name-corrections.md.
    ...
    # no ordinal. See docs/observance-name-corrections.md.
...
def _apply_position_label(label: str, d: datetime.date) -> str:
    ...
    if any(_is_position_component(p) for p in parts):
        return _OBSERVANCE_SEP.join(parts) if parts else label
    return _OBSERVANCE_SEP.join([position] + parts)
...
def _apply_eve_label(label: str, d: datetime.date) -> str:
    ...
    if any(p.startswith("Eve of ") for p in parts):
        return _OBSERVANCE_SEP.join(parts) if parts else label
    return _OBSERVANCE_SEP.join(parts + [eve]) if parts else eve
