...
class TestDecemberNinthFastMarker(unittest.TestCase):
    ...
    def test_marker_is_fast_on_advent_fast_weekdays(self):
        ...
        2016 and 2018 are Fri and Sun-adjacent years where Heesnak falls far enough from
        Dec 9 that the Fast of St. James the bishop of Nisibis does not claim the day.
        """
    ...
    def test_marker_is_superseded_inside_the_nisibis_fast(self):
        ...
        the 27 supported years. See docs/feast-name-corrections.md section 6.
        """
        for year, ordinal in ((2015, "Third"), (2020, "Third"), (2026, "Third")):
            d = datetime.date(year, 12, 9)
            with self.subTest(year=year):
                label = compute_armenian_lectionary(d)["Liturgical Day"]
                self.assertEqual(
