...
could not miss. It is now a tuple of thirteen names (``engine._TIERS``), and reordering it
...
* every adapter is ON the ladder, once, in the order it is defined -- an adapter added but
  never wired into ``_TIERS`` is dead code nothing else would notice, and definition order
...
def _applicable(d):
    ...
    return [tier for tier in engine._TIERS if tier(d) is not None]
...
# returns a _TierResult for the same date and is passed over. A pin whose loser does NOT
...
# relative order in _TIERS is unobservable.
#
...
# the only swaps in _TIERS that change any served day at all.
PRECEDENCE_PINS = (
...
class TestTheLadderIsComplete(unittest.TestCase):
    """``_TIERS`` is the ladder. An adapter missing from it is not a tier at all."""
    ...
    def test_every_adapter_is_on_the_ladder_in_definition_order(self):
        """Defined-but-unwired is the failure mode: an adapter absent from _TIERS never
        ...
        free."""
        self.assertEqual(list(engine._TIERS), self.adapters)

    def test_no_adapter_appears_twice(self):
        self.assertEqual(len(set(engine._TIERS)), len(engine._TIERS))

    def test_the_fallback_is_last(self):
        """``_tier_fallback``'s own comment says it "must stay last in _TIERS" because it
        never returns None. Anything after it is unreachable; the comment was the only
        thing saying so."""
        self.assertIs(engine._TIERS[-1], engine._tier_fallback)

    def test_the_fallback_is_unconditional(self):
        """Which is what makes "last" load-bearing, and what keeps
        ``_compute_lectionary``'s ``for``/``else`` RuntimeError unreachable. Includes
        dates outside MIN_YEAR-MAX_YEAR: ``_compute_lectionary`` has no range guard, and
        ``dev/build_observance_catalog`` reaches past the range through it."""
...
class TestPrecedenceIsPinned(unittest.TestCase):
    ...
    def test_the_stated_winner_wins(self):
        for iso, winner, loser, _source in PRECEDENCE_PINS:
            with self.subTest(date=iso, winner=winner, loser=loser):
                ...
                self.assertLess(engine._TIERS.index(_tier(winner)),
                                engine._TIERS.index(_tier(loser)))

    def test_every_pin_actually_contends(self):
        """A pin whose loser does not apply on that date would pass however _TIERS is
    ...
    def test_the_pins_cover_every_contended_adjacency(self):
        ...
        adjacent = [(engine._TIERS[i].__name__, engine._TIERS[i + 1].__name__)
                    for i in range(len(engine._TIERS) - 1)]
