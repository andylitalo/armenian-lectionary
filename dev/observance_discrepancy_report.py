...
``tests/test_observance.py`` compares only the *commemoration component* of the feast name, so
...
Markdown inventory to ``reports/observance_name_discrepancies.md``.

Each day's feast string is a list of ``_OBSERVANCE_SEP``-joined components (a calendar-position
...
Usage:
    python dev/observance_discrepancy_report.py            # -> reports/observance_name_discrepancies.md
    python dev/observance_discrepancy_report.py --stdout   # print instead of writing
"""
...
OUT_PATH = os.path.join(
    ...
    "reports", "observance_name_discrepancies.md")
...
def reconciled_components(iso, feast_str):
    ...
    expected_fast_marker_components and docs/observance-name-corrections.md. A day with no
...
def render(data):
    ...
    w("# Observance-name discrepancies: engine vs. sacredtradition.am\n")
    w(f"Generated by `dev/observance_discrepancy_report.py` on "
    ...
    w("Compares the **raw `\"Liturgical Day\"` string** — the value bahk persists into "
      "`Feast.name` — component by component on the `—` separator. `tests/test_observance.py` "
    ...
    w(f"No day in **{', '.join(missing_years) or '(none)'}** carries a source feast name, so "
      ...
      "above, and `tests/test_observance_contract.py`).\n")
